...
# Лабораторная работа «Управление процессами и потоками в Linux»
...
* **Аппаратная платформа:** 1 × AMD Ryzen 7 5800H @ 3.20 ГГц (испытания проводились на *одном ядре* с помощью `taskset -c 0`).
* **ОС:** Arch Linux 6.7.x, glibc 2.39, gcc 13.
...
*Пример вывода логов:* см. `lab2.log` citeturn3file1
...
* **Глобальная переменная и FILE**\* — пример `task_4_3.c` citeturn3file17.
...
| ------- | -------------------------------------------------------------------- | ----------------------------------------------------- |
| **5.1** | политика/приоритет по умолчанию (`SCHED_OTHER`, nice 0)              | `scheduler_info.c`, `experiment.c` citeturn3file11 |
| **5.2** | изменение nice и реакция `getpriority()`                             | `experiment_5_2.c` citeturn3file8                  |
| **5.3** | `SCHED_FIFO`, равные / разные приоритеты, границы 1‑99               | `experiment_fifo.c` citeturn3file14                |
| **5.4** | `SCHED_RR`, квант (`sched_rr_get_interval` ≈ 20 ms), `sched_yield()` | `experiment_5_4.c` citeturn3file9                  |
| **5.5** | конкуренция RR vs OTHER при одинаковом приоритете                    | `experiment_5_5.c` citeturn3file12                 |
| **5.6** | планирование внутри *потоков* одного процесса                        | `task_5_6.c` citeturn3file15                       |
...
| ------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------- |
| **6.1** | общий fd при `fork()/exec()` | файл открыт в родителе → доступен после `exec` citeturn3file5                                            |
| **6.2** | политика планирования        | `SCHED_OTHER` наследуется и после `exec` citeturn3file7                                                  |
| **6.3** | диспозиции сигналов          | custom‑handler сброшен после `exec`, IGN остался citeturn3file4                                          |
| **6.4** | `clone()` и CLONE\_VM        | с `CLONE_VM` глобальная переменная изменилась и в родителе; без флага изменение локально citeturn3file10 |
